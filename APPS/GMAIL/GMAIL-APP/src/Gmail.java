...
public class Gmail {
    ...
    private void draftModule(User user) {
        ...
        if (resp.equalsIgnoreCase("YES")) {
            ...
            if (opt == 1) {
                ...
                String toUserMail = editSend.getReciverMail();
    ...
    private void editDraft(Mail editSend) {
        ...
        for (User ele : userList) {
            if (editSend.getReciverMail().equals(ele.getMail())) {
                toUser = ele;
                break;
            }
        }
                ...
                outerLoop: for (;;) {
                    ...
                    for (User ele : userList) {
                        if (toMail.equals(ele.getMail())) {
                            editSend.setReciverMail(toMail);
                            toUser = ele;
                            break outerLoop;
                        }
                    }
        ...
        System.out.println("MAIL HAS BEEN SEND SUCCESFULLY FROM DRAFT");
    }
    ...
    private void composeMail(User user){
        ...
        Mail newMail =
            new Mail(user.getMail(),toUser.getMail(),subject,body);
